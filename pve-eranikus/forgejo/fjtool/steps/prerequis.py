...
provoquer un redémarrage. Les quatre pièges encodés ici viennent tous de la
production du CT 200 ; ils ne dépendent pas du service, seulement de LXC.
...
remettre ne produit aucune erreur, ne se voit pas, et laisse la source de
vérité sans son garde-fou. D'où un gestionnaire de contexte avec un `finally`,
jamais une paire d'appels.
...
l'effet `ct.reboot`, que le parcours coalesce et vide après toutes les étapes.

**`nesting=1` est obligatoire sur Debian 13.** Sans lui, les unités qui
montent un tmpfs pour les credentials systemd — ce que font `forgejo.service`
et `fj-backup.service` avec `PrivateTmp` — échouent en `243/CREDENTIALS`, et
le conteneur démarre en état dégradé sans que rien ne le signale.

**Un `mp2` monté ailleurs ne se déplace pas.** Il porte des sauvegardes. On le
...
class Protection:
    ...
    def check(self, ctx) -> Outcome:
        ...
        return Outcome(
            "drift",
            "désactivée — ce conteneur est la source de vérité d'ArgoCD",
            (
                Action(
...
class Nesting(EtapeA):
    ...
    def check(self, ctx) -> Outcome:
        ...
        return Outcome(
            ...
            (
                Action(
                    f"pct set {ctx.opts.ctid} --features nesting=1 "
...
class Mp1Depot(PointDeMontage):
    ...
    à voir les scripts du nœud, le nom du bucket ni le chemin de la clé GCP.

    `ro=1` n'est pas une commodité. Ce montage porte `app.ini` et le fichier
    `VERSION` : si le conteneur pouvait y écrire, une instance Forgejo
    compromise réécrirait sa propre configuration — et l'épinglage avec.
    """
    ...
    def check(self, ctx) -> Outcome:
        ...
        if voulu.matches(actuel):
            return Outcome("ok", f"{voulu.source} → {MP} (ro)")
...
class Startup(EtapeA):
    ...
    Pour cette instance-ci, ce n'est pas une formalité : elle est la source de
    vérité d'ArgoCD, donc ce qui doit être debout AVANT que le reste ne
    cherche à se réconcilier. `order=1` la place en tête.
    """
    ...
    def check(self, ctx) -> Outcome:
        ...
        return Outcome(
            "absent",
            "aucun ordre de démarrage — la source de vérité doit remonter en tête",
            (
                Action(
...
class Onboot(EtapeA):
    """Démarrage automatique au boot du nœud.

    `startup` dit dans quel ORDRE ; `onboot` dit S'IL démarre. Les deux sont
    indépendants, et un CT avec un ordre mais sans onboot ne démarre jamais —
    l'ordre s'appliquant à ceux qui démarrent.
    """

    name = "onboot"

    def check(self, ctx) -> Outcome:
        if self._ct(ctx).config().get("onboot") == "1":
            return Outcome("ok", "1")
        return Outcome(
            "absent",
            "le CT ne redémarrerait pas avec le nœud",
            (
                Action(
                    f"pct set {ctx.opts.ctid} --onboot 1",
                    lambda c: Container(c.runner, c.opts.ctid).set(onboot=1),
                ),
            ),
        )
